...
```dart
// "Book me" pages: the QR form (/book/) and the student's private status page (/book/status/?t=…).
...
- [ ] **Step 8 (controller):** `cd apps/site && dart analyze && dart test`. Expected: `No issues found!` and `+9: All tests passed!`.
